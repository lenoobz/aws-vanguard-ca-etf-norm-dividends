...
// DistributionMongo struct
type DistributionMongo struct {
	db     *mongo.Database
	client *mongo.Client
...
// NewDistributionMongo creates new distribution mongo repo
func NewDistributionMongo(db *mongo.Database, log logger.ContextLog, conf *config.MongoConfig) (*DistributionMongo, error) {
	if db != nil {
		return &DistributionMongo{
			db:   db,
			log:  log,
			conf: conf,
		}, nil
...
func (r *DistributionMongo) Close() {
...
// FindFundDistributions finds fund distributions
func (r *DistributionMongo) FindFundDistributions(ctx context.Context) ([]*entities.FundDistribution, error) {
	// create new context for the query
